...
function Gallery({ product }) {
  ...
  const images = [product.swatch, [product.swatch[1], product.swatch[0]], product.swatch];
  return (
    <div style={{
      position: "relative",
      aspectRatio: "1 / 1",
      ...
      {images.length > 1 && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ width: 1, height: 24, background: "var(--ink-30)", margin: "0 4px" }} />
            <button
            ...
            {opt.values.map((v, i) => {
              ...
              return (
                <button
                  key={v}
                  disabled={isDisabled}
                  onClick={() => !isDisabled && onChange({ ...selected, [opt.name]: v })}
                  style={{
                    minWidth: 48,
                    padding: "6px 14px",
...
function AddToCart({ product, selected, onAdd }) {
  ...
  const allSelected = !hasOptions ||
    product.variants.every((o) => selected[o.name]);
  ...
  return (
    ...
    >
      <div style={{ position: "absolute", left: 0, marginLeft: 16, display: "grid", placeItems: "center" }}>
        <IconPlus />
      </div>
      Add to cart
    </button>
...
function ProductPage({ product, onAdd }) {
  const [selected, setSelected] = React.useState(() => {
    ...
    const s = {};
    (product.variants || []).forEach((o) => { s[o.name] = o.values[0]; });
    return s;
  });
  React.useEffect(() => {
    const s = {};
      ...
      <div style={{
        ...
        <div style={{ flex: "0 0 calc(40% - 40px)" }}>
          <div style={{
            ...
            <PriceTag amount={product.price} salePrice={product.salePrice} showCurrency />
          </div>
          <VariantSelector

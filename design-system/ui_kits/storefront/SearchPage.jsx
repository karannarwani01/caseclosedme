          ...
          return (
            <li key={item.title}>
              <a
                ...
                onMouseEnter={(e) =>
                  !isActive && (e.currentTarget.style.color = "var(--accent)")
                }
                onMouseLeave={(e) =>
                  !isActive && (e.currentTarget.style.color = "var(--fg-2)")
                }
              >
                {item.title}
              </a>
...
function SearchPage({ query, collection, sort, onNavigate, onSelect }) {
  ...
  if (collection) {
    ...
    if (cat && cat.title !== "All products") {
      if (cat.slug === "just-dropped")
        results = results.filter((p) => p.badge === "New");
      else if (cat.slug === "sale")
        results = results.filter((p) => p.salePrice);
    ...
    >
      ...
      <div>
        ...
        {results.length > 0 ? (
          ...
          >
            Try a broader term, or browse trending.
          </div>
        )}
      </div>
      ...
      <aside>
        <FilterList
          ...
          activeSlug={sort}
          onSelect={(slug) =>
            onNavigate(collection ? `search:${collection}` : "search", {
              q: query,
              sort: slug,
            })
          }

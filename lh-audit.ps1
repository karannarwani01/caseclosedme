...
foreach ($i in 1..3) {
  npx lighthouse http://localhost:3009 --only-categories=performance --form-factor=mobile --screenEmulation.mobile --chrome-flags="--headless=new" --output=json --output-path="lh-$Tag-$i.json" --quiet 2>$null | Out-Null

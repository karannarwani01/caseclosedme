...
function addMesh(name, g, material, translation) {
  const posB = f32(g.pos), nrmB = f32(g.nrm), idxB = pad(u16(g.idx));
  const start = bin.length;
  bin = Buffer.concat([bin, posB, nrmB, idxB]);

  const bv = (off, len, target) =>
    j.bufferViews.push({ buffer: 0, byteOffset: off, byteLength: len, target }) - 1;
  ...
  const lo = [0, 1, 2].map((k) => Math.min(...g.pos.filter((_, i) => i % 3 === k)));
  const hi = [0, 1, 2].map((k) => Math.max(...g.pos.filter((_, i) => i % 3 === k)));
  const aP = j.accessors.push({ bufferView: bvP, componentType: 5126, count: g.pos.length / 3, type: "VEC3", min: lo, max: hi }) - 1;
  const aN = j.accessors.push({ bufferView: bvN, componentType: 5126, count: g.nrm.length / 3, type: "VEC3" }) - 1;
  const aI = j.accessors.push({ bufferView: bvI, componentType: 5123, count: g.idx.length, type: "SCALAR" }) - 1;

  const mesh = j.meshes.push({
    name,
    primitives: [{ attributes: { POSITION: aP, NORMAL: aN }, indices: aI, material }],
  }) - 1;
...
addMesh(
  "Stacking lip",
  boxGeom(lid.min[0] + ix, lid.max[1], lid.min[2] + iz, lid.max[0] - ix, lid.max[1] + STEP, lid.max[2] - iz),
  mat,
  lidNode.translation || [0, 0, 0],
);

// Base: four bars forming a rim around the underside, leaving the middle recessed.
const [bx0, , bz0] = body.min, [bx1, , bz1] = body.max;
const y0 = body.min[1] - STEP, y1 = body.min[1];
const rim = [
...
rim.forEach((r, i) =>
  addMesh(`Stacking rim ${i + 1}`, boxGeom(...r), mat, bodyNode.translation || [0, 0, 0]),
);
...
const jh = Buffer.alloc(8); jh.writeUInt32LE(njson.length, 0); jh.write("JSON", 4);
...
const c0 = glb.readUInt32LE(12), c1 = glb.readUInt32LE(20 + c0);
console.log(
